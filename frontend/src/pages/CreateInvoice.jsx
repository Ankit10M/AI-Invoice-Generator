...
export default function CreateInvoice() {
  ...
  const isServiceInvoice = invoice && (invoice._id || invoice.id?.length === 24);
  ...
  const generateUniqueInvoiceNumber = useCallback(
    async (attempts = 10) => {
      for (let i = 0; i < attempts; i++) {
        ...
        const exists = isLoaded && isSignedIn ? await checkInvoiceExists(candidate) : false;
  ...
  }, [isLoaded, isSignedIn, obtainToken]);
  ...
  async function handleSave() {
    ...
    try {
      ...
      const prepared = {
        ...
        notes: invoice.notes || "",
        localId: invoice.localId || invoice.id,
      };
      ...
      const shouldUpdate = isEditing && isServiceInvoice;
      const endpoint =
        shouldUpdate
      ...
      if (res.status === 404 && method === 'PUT') {
        return await fetch(`${API_BASE}/api/invoice`, {
          method: 'POST',
          headers,
          body: JSON.stringify(prepared)
        })
