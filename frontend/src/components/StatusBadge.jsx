...
const StatusBadge = ({ status = '', size = 'default', showIcon = true }) => {
    ...
    return (
        <div className={`inline-flex items-center ${sizeClasses[size]} rounded-full font-medium ${config.bg} ${config.text} ${config.border} border transition-all duration-300 ease-out hover:scale-105 hover:shadow-sm group relative overflow-hidden`}>
            ...
            <span className='relative z-10 font-semibold tracking-wide first-letter:uppercase'>{s === 'default' ? status : s}</span>
            {(s === 'unpaid' || s === "overdue") && (
