...
export async function getInvoiceById(req, res) {
    try {
        ...
        if (!inv) return res.status(404).json({ message: 'Invoice not found', success: false })
        if (inv.owner && String(inv.owner) !== String(userId)) {
            return res.status(403).json({ message: 'forbidden: not your Invoice ', success: false })
        }
...
export async function updateInvoice(req, res) {
    try {
        ...
        if (!updated) return res.status(500).json({ message: 'Failed to update invoice ', success: false })
        return res.status(200).json({ message: 'Invoice Updted', data: updated })
    } catch (error) {
...
export async function deleteInvoice(req, res) {
    try {
        ...
        return res.status(200).json({ message: 'Invoice deleted', success: true })
    } catch (error) {
