...
aiInvoiceRoute.post('/generate', async (req, res) => {
    try {
        if (!API_KEY) return res.status(500).json({ message: 'Server failed no key found', success: false })
        const { prompt } = req.body;
        if (!prompt || !prompt.trim()) {
            return res.status(400).json({ success: false, message: 'prompt text Required' })
        }
        ...
        try {
            ...
            return res.status(502).json({
                message: ' ai return invalid json', success: false, model: usedModel, raw: text
            })
        }
        return res.status(200).json({ model: usedModel, success: true, data })
    } catch (error) {
        console.error('Ai generation failed', error);
        return res.status(500).json({ success: false, message: 'server error', detail: error?.message || String(error) })
    }
})
